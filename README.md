...
> **Status: built and working.** Streamlit app + deterministic Python core + optional Claude narrative layer, with a synthetic demo dataset and a 33-test suite. The app starts **empty**: you drop source files in live, and the copilot analyses each one as it lands — streaming its findings (real engine output, narrated) before the dashboards light up. See [Quickstart](#quickstart).
...
5. **Readiness memo & export (Tab 5).** The memo doesn't pre-exist — click **"Draft readiness memo"** and a workpaper-style memo (executive summary, blockers, owner-by-owner data requests, open questions, full mapping appendix) streams onto the screen, alongside a gap-register CSV and a JSON audit package. The final package is **gated** until all mappings are dispositioned and entities reconciled.

6. **Use your own data (📥 tab).** The app is not sample-bound: drop any CSV/XLSX extracts into the same intake zone and the same live analysis runs on them. The tab documents exactly what files/columns to request (entity master, trial balance, provision extract, CbC Table 1, jurisdiction attributes) and offers blank CSV templates whose headers map automatically.
...
beps-pillar-two-data-gap-copilot/
  app.py                          # Streamlit UI: live intake + 7 tabs
  scripts/make_sample_data.py     # regenerates the synthetic Atlas dataset
  ...
  src/pillar_two_copilot/
    ...
    narration.py                  # streams deterministic results as the live "copilot analysis"
...
| Missing jurisdiction | Atlas UK Services Ltd in entity master | high-severity gap; routine-profits test marked unevaluable for the UK |
